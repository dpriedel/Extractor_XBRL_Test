...
const EM::FileName FILE_WITH_XML_10Q{
    "/vol_DA/SEC/Archives/edgar/data/1460602/0001062993-13-005017.txt"};
const EM::FileName FILE_WITH_XML_10K{
    "/vol_DA/SEC/Archives/edgar/data/google-10k.txt"};
const EM::FileName FILE_WITHOUT_XML{
    "/vol_DA/SEC/Archives/edgar/data/841360/0001086380-13-000030.txt"};
const EM::FileName EDGAR_DIRECTORY{"/vol_DA/SEC/Archives/edgar/data"};
const EM::FileName FILE_NO_NAMESPACE_10Q{
    "/vol_DA/SEC/Archives/edgar/data/68270/0000068270-13-000059.txt"};
const EM::FileName FILE_SOME_NAMESPACE_10Q{
    "/vol_DA/SEC/Archives/edgar/data/1552979/0001214782-13-000386.txt"};
const EM::FileName FILE_MULTIPLE_LABEL_LINKS{
    "/vol_DA/SEC/Archives/edgar/data/1540334/0001078782-13-002015.txt"};
const EM::FileName BAD_FILE1{
    "/vol_DA/SEC/SEC_forms/0001000228/10-K/0001000228-11-000014.txt"};
const EM::FileName BAD_FILE2{
    "/vol_DA/SEC/SEC_forms/0001000180/10-K/0001000180-16-000068.txt"};
const EM::FileName BAD_FILE3{
    "/vol_DA/SEC/SEC_forms/0001000697/10-K/0000950123-11-018381.txt"};
const EM::FileName NO_SHARES_OUT{
    "/vol_DA/SEC/SEC_forms/0001023453/10-K/0001144204-12-017368.txt"};
const EM::FileName TEST_FILE_LIST{"./list_with_bad_file.txt"};
const EM::FileName MISSING_VALUES1_10K{
    "/vol_DA/SEC/SEC_forms/0001004980/10-K/0001193125-12-065537.txt"};
const EM::FileName MISSING_VALUES2_10K{
    "/vol_DA/SEC/SEC_forms/0001005210/10-Q_A/0001193125-12-335145.txt"};

const EM::FileName ORIGINAL_10Q{
    "/vol_DA/SEC/SEC_forms/0001001258/10-Q/0001193125-14-043453.txt"};
const EM::FileName AMENDED_10Q{
    "/vol_DA/SEC/SEC_forms/0001001258/10-Q_A/0001193125-15-234644.txt"};

const EM::FileName XLS_SHEET_1{
    "/vol_DA/SEC/SEC_forms/0001453883/10-K_A/0001079974-16-001022.txt"};
const EM::FileName XLS_SHEET_2{
    "/vol_DA/SEC/Archives/edgar/data/68270/0000068270-13-000059.txt"};
const EM::FileName XLS_SHEET_3{
    "/vol_DA/SEC/Archives/edgar/data/736822/0001437749-13-013064.txt"};
const EM::FileName XLS_SHEET_4{
    "/vol_DA/SEC/Archives/edgar/data/40888/0001193125-13-399898.txt"};
const EM::FileName XLS_SHEET_5{
    "/vol_DA/SEC/Archives/edgar/data/29989/0000029989-13-000015.txt"};
const EM::FileName XLS_SHEET_6{
    "/vol_DA/SEC/Archives/edgar/data/110471/0000110471-13-000005.txt"};
...
struct line_only_whitespace : std::ctype<char> {
  ...
  explicit line_only_whitespace(std::size_t refs = 0)
      : ctype(make_table(), false, refs) {}
};

// some utility functions for testing.

int FindAllLabels(const std::vector<EM::GAAP_Data> &gaap_data,
                  const EM::Extractor_Labels &labels) {
  if (gaap_data.empty()) {
...
bool FindAllContexts(const std::vector<EM::GAAP_Data> &gaap_data,
                     const EM::ContextPeriod &contexts) {
  if (gaap_data.empty()) {
    std::cout << "Empty GAAP data." << "\n";
    return false;
  }
...
void PrintRangeDifferences(const std::vector<EM::GAAP_Data> &rng1,
                           const std::vector<EM::GAAP_Data> &rng2) {
  std::cout << "Items in first range: " << rng1.size() << '\n';
  std::vector<EM::GAAP_Data> in_rng1_only;
  auto x = rng::set_difference(
      rng1, rng2, rng::back_inserter(in_rng1_only),
      [](const auto &lhs, const auto &rhs) { return lhs.label < rhs.label; });

  std::cout << "\n\nItems missing from amended form.\n\n";
  rng::for_each(in_rng1_only, [](const auto &e) {
    std::cout << e.label << " : " << e.context_ID << " : " << e.value << '\n';
  });

  std::cout << "Items in second range: " << rng2.size() << '\n';
  std::vector<EM::GAAP_Data> in_rng2_only;
  auto y = rng::set_difference(
      rng2, rng1, rng::back_inserter(in_rng2_only),
      [](const auto &lhs, const auto &rhs) { return lhs.label < rhs.label; });

  std::cout << "\n\nItems missing from original form.\n\n";
  rng::for_each(in_rng2_only, [](const auto &e) {
    std::cout << e.label << " : " << e.context_ID << " : " << e.value << '\n';
  });
}
...
TEST_F(LocateFileContentToUse, FindInstanceDocument10Q) {
  ...
  auto instance_document =
      LocateInstanceDocument(document_sections_10Q, FILE_WITH_XML_10Q);
...
TEST_F(LocateFileContentToUse, FindInstanceDocument10K) {
  ...
  auto instance_document =
      LocateInstanceDocument(document_sections_10K, FILE_WITH_XML_10K);
  ASSERT_TRUE(instance_document.get().starts_with("<?xml version") &&
...
TEST_F(LocateFileContentToUse, FindLabelDocument10Q) {
  ...
  auto labels_document =
      LocateLabelDocument(document_sections_10Q, FILE_WITH_XML_10Q);
...
TEST_F(LocateFileContentToUse, FindLabelDocument10K) {
  ...
  auto labels_document =
      LocateLabelDocument(document_sections_10K, FILE_WITH_XML_10K);
  ASSERT_TRUE(labels_document.get().starts_with("<?xml version") &&
              labels_document.get().ends_with("</link:linkbase>\n"));
}
...
TEST_F(ParseDocumentContent, VerifyParseBadInstanceDocumentThrows10K) {
  ...
  auto instance_document =
      LocateInstanceDocument(document_sections_10K, BAD_FILE1);
...
TEST_F(ParseDocumentContent, VerifyParseBadInstanceDocumentThrows210K) {
  ...
  auto instance_document =
      LocateInstanceDocument(document_sections_10K, BAD_FILE3);
  ASSERT_THROW(ParseXMLContent(instance_document), ExtractorException);
}
...
TEST_F(ExtractDocumentContent, VerifyCanExtractFilingDataNoSharesOut10K) {
  ...
  auto instance_document =
      LocateInstanceDocument(document_sections_10K, NO_SHARES_OUT);
  auto instance_xml = ParseXMLContent(instance_document);
...
TEST_F(ExtractDocumentContent, VerifyCanExtractGAAPNoNamespace10Q) {
  ...
  auto instance_document =
      LocateInstanceDocument(document_sections_10Q, FILE_NO_NAMESPACE_10Q);
  auto instance_xml = ParseXMLContent(instance_document);
...
TEST_F(ExtractDocumentContent, VerifyCanExtractLabelsNoNamespace10Q) {
  ...
  auto labels_document =
      LocateLabelDocument(document_sections_10Q, FILE_NO_NAMESPACE_10Q);
  auto labels_xml = ParseXMLContent(labels_document);
...
TEST_F(ExtractDocumentContent, VerifyCanExtractLabelsMultipleLabelLinks10Q) {
  ...
  auto labels_document =
      LocateLabelDocument(document_sections_10Q, FILE_MULTIPLE_LABEL_LINKS);
  auto labels_xml = ParseXMLContent(labels_document);
...
TEST_F(ExtractDocumentContent, VerifyCanExtractContextsSomeNamespace10Q) {
  ...
  auto instance_document =
      LocateInstanceDocument(document_sections_10Q, FILE_SOME_NAMESPACE_10Q);
  auto instance_xml = ParseXMLContent(instance_document);
...
TEST_F(ExtractDocumentContent, VerifyCanMatchGAAPDataWithUserLabelBadFile210K) {
  ...
  auto instance_document =
      LocateInstanceDocument(document_sections_10K, BAD_FILE2);
  auto instance_xml = ParseXMLContent(instance_document);
...
TEST_F(ExtractDocumentContent,
       VerifyCanMatchGAAPDataWithUserLabelNoNamespace10Q) {
  auto file_content_10Q = LoadDataFileForUse(FILE_NO_NAMESPACE_10Q);
  EM::FileContent file_content{file_content_10Q};
...
TEST_F(ExtractDocumentContent,
       VerifyCanMatchGAAPDataWithUserLabelSomeNamespace10Q) {
  auto file_content_10Q = LoadDataFileForUse(FILE_SOME_NAMESPACE_10Q);
  EM::FileContent file_content{file_content_10Q};
  ...
  auto labels_document =
      LocateLabelDocument(document_sections_10Q, FILE_SOME_NAMESPACE_10Q);
  auto labels_xml = ParseXMLContent(labels_document);
...
TEST_F(ExtractDocumentContent,
       VerifyCanMatchGAAPDataWithUserLabelMissingValues10K) {
  ...
  auto labels_document =
      LocateLabelDocument(document_sections_10K, MISSING_VALUES1_10K);
  auto labels_xml = ParseXMLContent(labels_document);

  auto label_data = ExtractFieldLabels(labels_xml);

  auto instance_document =
      LocateInstanceDocument(document_sections_10K, MISSING_VALUES1_10K);
  auto instance_xml = ParseXMLContent(instance_document);
...
TEST_F(ExtractDocumentContent,
       DISABLED_VerifyCanMatchGAAPDataWithUserLabelMissingValues210K) {
  ...
  auto labels_document =
      LocateLabelDocument(document_sections_10K, MISSING_VALUES2_10K);
  auto labels_xml = ParseXMLContent(labels_document);
  ...
  auto instance_document =
      LocateInstanceDocument(document_sections_10K, MISSING_VALUES2_10K);
  auto instance_xml = ParseXMLContent(instance_document);
...
TEST_F(ExtractDocumentContent,
       VerifyCanMatchGAAPDataWithContextSomeNamespace10Q) {
  auto file_content_10Q = LoadDataFileForUse(FILE_SOME_NAMESPACE_10Q);
  EM::FileContent file_content{file_content_10Q};
...
TEST_F(ValidateFolderFilters, VerifyFindAllXBRL) {
  ...
  auto test_file([&files_with_XML](const auto &dir_ent) {
    if (dir_ent.status().type() == fs::file_type::regular) {
      auto file_content_dir =
          LoadDataFileForUse(EM::FileName{dir_ent.path().c_str()});
  ...
  std::for_each(fs::recursive_directory_iterator(EDGAR_DIRECTORY.get()),
                fs::recursive_directory_iterator(), test_file);
...
TEST_F(ValidateFolderFilters, VerifyFindAll10Q) {
  ...
  auto test_file([&files_with_form](const auto &dir_ent) {
    if (dir_ent.status().type() == fs::file_type::regular) {
      ...
      bool has_form =
          ApplyFilters(SEC_data.GetFields(), sections, filter1, filter2);
      if (has_form) {
        ++files_with_form;
      }
    }
...
TEST_F(ValidateFolderFilters, VerifyFindAllInDateRange) {
  ...
  auto test_file([&files_with_form](const auto &dir_ent) {
    if (dir_ent.status().type() == fs::file_type::regular) {
      ...
      FileIsWithinDateRange filter2{year_month_day{2013_y / January / 1},
                                    year_month_day{2013_y / 9 / 30}};
...
TEST_F(ValidateFolderFilters, VerifyFindAllInDateRangeNoMatches) {
  ...
  auto test_file([&files_with_form](const auto &dir_ent) {
    if (dir_ent.status().type() == fs::file_type::regular) {
      ...
      using namespace date;
      FileHasXBRL filter1;
      FileIsWithinDateRange filter2{year_month_day{2015_y / January / 1},
                                    year_month_day{2015_y / 9 / 30}};
...
TEST_F(ValidateFolderFilters, VerifyComboFiltersWithMatches) {
  ...
  auto test_file([&files_with_form](const auto &dir_ent) {
    if (dir_ent.status().type() == fs::file_type::regular) {
      ...
      std::vector<std::string> forms{"10-Q"};
      FileHasFormType filter2{forms};
      FileIsWithinDateRange filter3{year_month_day{2013_y / 3 / 1},
                                    year_month_day{2013_y / 3 / 31}};
...
TEST_F(ValidateFolderFilters, VerifyFindFormsInFileNameList) {
  ...
  input_file.imbue(
      std::locale(input_file.getloc(), new line_only_whitespace()));
  ...
  auto qs = std::count_if(
      std::begin(list_of_files_to_process), std::end(list_of_files_to_process),
      [&forms1](const auto &fname) {
        return FormIsInFileName(forms1, EM::FileName{fname});
      });
  EXPECT_EQ(qs, 114);

  std::vector<std::string> forms2{"10-K"};
  auto ks = std::count_if(
      std::begin(list_of_files_to_process), std::end(list_of_files_to_process),
      [&forms2](const auto &fname) {
        return FormIsInFileName(forms2, EM::FileName{fname});
      });
  EXPECT_EQ(ks, 25);

  std::vector<std::string> forms3{"10-K", "10-Q"};
  auto kqs = std::count_if(
      std::begin(list_of_files_to_process), std::end(list_of_files_to_process),
      [&forms3](const auto &fname) {
        return FormIsInFileName(forms3, EM::FileName{fname});
      });
  ASSERT_EQ(kqs, 139);
}
...
TEST_F(ProcessAmendedForms, CanReadAmended10Q) {
  ...
  auto labels_document =
      LocateLabelDocument(document_sections_10Q, AMENDED_10Q);
  auto labels_xml = ParseXMLContent(labels_document);

  auto label_data = ExtractFieldLabels(labels_xml);

  auto instance_document =
      LocateInstanceDocument(document_sections_10Q, AMENDED_10Q);
  auto instance_xml = ParseXMLContent(instance_document);
...
TEST_F(ProcessAmendedForms, CompareOriginalAndAmended10Qs) {
  ...
  const auto orig_document_sections_10Q{
      LocateDocumentSections(orig_file_content)};

  auto orig_labels_document =
      LocateLabelDocument(orig_document_sections_10Q, ORIGINAL_10Q);
  auto orig_labels_xml = ParseXMLContent(orig_labels_document);

  auto orig_label_data = ExtractFieldLabels(orig_labels_xml);

  auto orig_instance_document =
      LocateInstanceDocument(orig_document_sections_10Q, ORIGINAL_10Q);
  ...
  orig_gaap_data = std::move(orig_gaap_data) |
                   rng::actions::sort([](const auto &lhs, const auto &rhs) {
  ...
  ASSERT_FALSE(rng::equal(
      orig_gaap_data, gaap_data, [](const auto &lhs, const auto &rhs) {
...
TEST_F(ProcessXLSXContent, CanProcess10KAmendedFile1) {
  ...
  auto bal_sheets = rng::find_if(xls_file, [](const auto &x) {
    return x.GetSheetName() == "balance sheets";
  });
  ...
  auto stmt_of_ops = rng::find_if(xls_file, [](const auto &x) {
    return x.GetSheetName() == "statements of operations";
  });
  ...
  auto cash_flows = rng::find_if(xls_file, [](const auto &x) {
    return x.GetSheetName() == "statements of cash flows";
  });
...
TEST_F(ProcessXLSXContent, CanProcess10QFile1) {
  ...
  auto stmt_of_ops = rng::find_if(xls_file, [](const auto &x) {
    return x.GetSheetNameFromInside().find("statements of operations") !=
           std::string::npos;
  });
  EXPECT_TRUE(stmt_of_ops != rng::end(xls_file));
  ...
  auto cash_flows = rng::find_if(xls_file, [](const auto &x) {
    return x.GetSheetNameFromInside().find("statements of cash flows") !=
           std::string::npos;
  });
...
TEST_F(ProcessXLSXContent, CanFindSharesOutstanding) {
  ...
  auto xls_content_10Q2 =
      LocateXLSDocument(document_sections_10Q2, XLS_SHEET_3);
...
TEST_F(ProcessXLSXContent, CanProcess10QFile1HighLevel) {
  ...
  auto financial_content =
      FindAndExtractXLSContent(document_sections_10Q, XLS_SHEET_2);
  EXPECT_TRUE(financial_content.has_data());

  EXPECT_EQ(financial_content.balance_sheet_.values_.size(), 33);
  rng::for_each(financial_content.balance_sheet_.values_, [](const auto &row) {
  ...
  rng::for_each(financial_content.cash_flows_.values_, [](const auto &row) {
    std::cout << row.first << '\t' << row.second << '\n';
  });
  std::cout << "\n\n\n";
}
...
TEST_F(ProcessXLSXContent, CanProcess10QFile1WithNotesHighLevel) {
  ...
  auto cash_flows = rng::find_if(xls_file, [](const auto &x) {
    return x.GetSheetNameFromInside().find("cash flow") != std::string::npos;
  });
  EXPECT_TRUE(cash_flows != rng::end(xls_file));
  int rows = rng::distance(*cash_flows);
  EXPECT_EQ(rows, 47);
  rng::for_each(*cash_flows, [](const auto &row) {
